...
# --- CONSTANTS RENDER ---
# IMPROVEMENT: Lower resolution for judging (faster), high resolution for final
JUDGE_IMG_WIDTH = 256  # Lower res for faster judging
JUDGE_IMG_HEIGHT = 256
FINAL_IMG_WIDTH = 518   # High res for final output
FINAL_IMG_HEIGHT = 518

IMG_WIDTH = JUDGE_IMG_WIDTH  # Default for judging
IMG_HEIGHT = JUDGE_IMG_HEIGHT
GRID_VIEW_GAP = 5
VIEWS_NUMBER = 16
...
# --- CONSTANTS JUDGE ---
SYSTEM_PROMPT = """
You are an expert 3D model evaluator specializing in:
- Geometric accuracy and shape fidelity to the reference image
- Texture quality, detail preservation, and visual realism
- Visual coherence, completeness, and proper object structure
- Prompt adherence and object category correctness

Evaluate with precision and provide detailed analysis.
Always respond with valid JSON only."""
USER_PROMPT_IMAGE = """Evaluate how well each 3D model matches the reference image prompt.

Consider:
1. Shape accuracy: Does the 3D model match the object's shape in the image?
2. Detail preservation: Are important details (textures, patterns, features) preserved?
3. Completeness: Is the object complete without missing parts?
4. Visual quality: Is the model visually coherent and realistic?
5. Category match: Does it represent the same type of object?

Penalty Scale (0-10):
0 = Perfect match - identical shape, details, and quality
1-2 = Excellent - minor differences, high quality
3-4 = Good - some differences but acceptable quality
5-6 = Moderate - noticeable differences, quality issues
7-8 = Poor - significant differences, low quality
9-10 = Very poor - wrong object or completely different

Output: {"penalty_1": <0-10>, "penalty_2": <0-10>, "issues": "<brief explanation>"}"""
...
class DuelManager:
    ...
    async def get_penalty_score(self, prompt_bytes: bytes, rendered_image_bytes: bytes) -> float:
        """
        IMPROVEMENT: Get penalty score for a single candidate (for early stopping)
        Returns penalty score (0-10, lower is better)
        Uses a simplified evaluation against the prompt
        """
        if not rendered_image_bytes:
            return 10.0  # Worst score if invalid
        
        prompt_b64 = base64.b64encode(prompt_bytes).decode('utf-8').strip()
        render_b64 = base64.b64encode(rendered_image_bytes).decode('utf-8').strip()
        ...
        try:
            messages = [
                {"role": "system", "content": SYSTEM_PROMPT},
                {
                    ...
                    "content": [
                        ...
                        {"type": "image_url", "image_url": {"url": f"data:image/png;base64,{prompt_b64}"}},
                        {"type": "text", "text": "3D model (4 different views):"},
                        {"type": "image_url", "image_url": {"url": f"data:image/png;base64,{render_b64}"}},
                        {"type": "text", "text": "Rate this 3D model quality (penalty 0-10, lower is better):\nOutput: {\"penalty\": <0-10>, \"issues\": \"<brief>\"}"},
                    ],
                },
            ]
            ...
            if content:
                try:
                    ...
                    return float(result.get("penalty", 5.0))
                except json.JSONDecodeError:
                    return 5.0
            return 5.0
        except Exception as e:
            logger.error(f"Failed to get penalty score: {e}")
