...
async def run_champion_generation(
    ...
    logger.info(f"⚔️ VLLM judging {num_texture_candidates} texture candidates (PARALLEL + EARLY STOP)...")
    
    best_idx = 0
    
    async def judge_all_candidates_parallel():
        """
        IMPROVEMENT: Parallel judging thay vì sequential
        Judge tất cả candidates vs baseline cùng lúc, với early stopping
        """
        nonlocal best_idx
        
        if num_texture_candidates == 1:
            return
        
        # Parallel judge all other candidates vs baseline
        judge_tasks = []
        for i in range(1, num_texture_candidates):
            task = duel_manager.run_duel_prerendered(
                image_bytes,
                rendered_images[best_idx],  # Baseline
                rendered_images[i]           # Candidate
            )
            judge_tasks.append((i, task))
        
        # Execute all duels in parallel
        logger.info(f"   🚀 Judging {len(judge_tasks)} candidates in parallel...")
        results = await asyncio.gather(*[task for _, task in judge_tasks], return_exceptions=True)
        
        # Process results and find best
        for (i, _), result in zip(judge_tasks, results):
            if isinstance(result, Exception):
                logger.error(f"   Candidate {i+1}: Judging failed: {result}")
                continue
            
            winner_idx, issues = result
            if winner_idx == 1:  # Current candidate won
                logger.info(f"   Candidate {i+1}: BETTER than candidate {best_idx+1}")
                best_idx = i
                
                # EARLY STOPPING: Check if this candidate is excellent
                # Get penalty score for early stopping check
                try:
                    candidate_penalty = await duel_manager.get_penalty_score(
                        image_bytes, rendered_images[i]
                    )
                    if candidate_penalty < 2.0:
                        logger.success(f"   ✅ Found excellent candidate {i+1} (penalty: {candidate_penalty:.2f}), early stopping!")
                        break
                except Exception as e:
                    logger.debug(f"   Could not get penalty score: {e}")
                    # Continue without early stopping if score check fails
            else:
                logger.info(f"   Candidate {i+1}: Worse than candidate {best_idx+1}")
    
    # Run judging with adaptive timeout
    remaining_time = max(3, settings.generation_timeout - elapsed)  # At least 3 seconds for parallel
    try:
        await asyncio.wait_for(judge_all_candidates_parallel(), timeout=remaining_time)
